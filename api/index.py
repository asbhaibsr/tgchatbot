...
from handlers.user import (
    ...
    report_handler, callback_handler,
)
...
def get_bot_app():
    ...
    # ── Movie/Document file handler (premium obfuscation) ─────────────
    application.add_handler(MessageHandler(
        filters.Document.ALL | filters.VIDEO,
        movie_file_handler
    ))

    # ── Main chat handler (LAST — sabse neeche rehna chahiye) ─────────
    application.add_handler(MessageHandler(
...
@app.get("/")
async def root():
    return {"status": f"{BOT_TOKEN and 'Bot running' or 'No token'} 💘", "token_ok": bool(BOT_TOKEN)}
